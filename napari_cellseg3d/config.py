...
class PostProcessConfig:
    ...
    zoom: Zoom = field(default_factory=lambda:Zoom())
    thresholding: Thresholding = field(default_factory=lambda:Thresholding())
    instance: InstanceSegConfig = field(default_factory=lambda:InstanceSegConfig())
...
class InferenceWorkerConfig:
    ...
    device: str = "cpu"
    model_info: ModelInfo = field(default_factory=lambda:ModelInfo())
    weights_config: WeightsInfo = field(default_factory=lambda:WeightsInfo())
    results_path: str = str(Path.home() / "cellseg3d" / "inference")
    filetype: str = ".tif"
    keep_on_cpu: bool = False
    compute_stats: bool = False
    post_process_config: PostProcessConfig = field(default_factory=lambda:PostProcessConfig())
    sliding_window_config: SlidingWindowConfig = field(default_factory=lambda:SlidingWindowConfig())
    use_crf: bool = False
    crf_config: CRFConfig = field(default_factory=lambda:CRFConfig())
...
class TrainingWorkerConfig:
    ...
    batch_size: int = 1
    deterministic_config: DeterministicConfig = field(default_factory=lambda:DeterministicConfig())
    scheduler_factor: float = 0.5
    scheduler_patience: int = 10
    weights_info: WeightsInfo = field(default_factory=lambda:WeightsInfo())
    # data params
...
class WNetTrainingWorkerConfig(TrainingWorkerConfig):
    ...
    input_brightness_range: tuple[float, float] = None
